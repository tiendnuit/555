...
class SHA1 {
    ...
    func prepare(_ len:Int = 64) -> NSMutableData {
        let tmpMessage: NSMutableData = NSData(data: self.message) as Data as Data as! NSMutableData
    ...
    func calculate() -> Data {
        ...
        let sequence = stride(from: 0, to: tmpMessage.length, by: chunkSizeBytes)
        for i in sequence{
